...
const DocUpdateAppt = () => {
    ...
    const fetchAppointment = async () => {
      try {
        const response = await axios.get(
          `https://absh.onrender.com/api/v1/appointment/get_appointment?id=${appointment_id}`
        );
        const appointment = response.data.data;
        const scheduledTime = new Date(appointment.scheduled_time);
        setTime(scheduledTime);
        setCompleted(appointment.is_completed);
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        console.log("Submitting with data:", { appointment_id, scheduled_time, is_completed });
      ...
      setLoading(false);
      Swal.fire("Your Appointment has been updated successfully. Thank you!");
    } catch (err) {
      if (!err?.response) {
            ...
            <div className="card-body">
              <form className="row g-3" onSubmit={handleSubmit}>
                ...
                <div className="form-outline mb-4">
                  ...
                  <label htmlFor="inputCompleted" className="form-label">
                    Completed
                  </label>
                </div>
